...
**Phase:** Phase 3.2 verified on iPhone (Andrew confirmed — Dashboard tab renders under shell, scroll preserved, cross-tab nav works, no React #418/#423). Phase 3.3 shipped pending push: BetsTab migrated following the Phase 3.2 recipe (1 file new + 1 re-export + 1 AppShell mount). Variant noted: BetsTab uses early-return-from-no-params instead of one-shot ref because filter consume can fire multiple times legitimately. Capacitor `<a href="/route">` quirk surfaced during 3.2 iPhone test logged under new "PRE-EXISTING NATIVE ROUTING BUGS" section. typecheck + build:mobile clean.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's spot-check of Phase 3.3 diff (per WORKFLOW PROTOCOL one-commit-per-phase). Light iPhone spot-check optional after diff approval.
...
**Phase 3.4-3.7 — see Key Changes below (per-phase "go" still required for each).**
