...
**Phase:** Phase 2 verification gate closed (A/B/C/D all pass on physical iPhone, 2026-05-09). Phase 3 recon complete + 4 deliverables surfaced + Andrew approved with 5 additions. Phase 3.0 (React #418/#423 fix) shipped pending push: AuthProvider cache read moved from useState initializer to useIsomorphicLayoutEffect. typecheck + build:mobile clean.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's spot-check of Phase 3.0 diff + iPhone re-test (verify React #418/#423 disappear from the Capacitor webview console on cold open of the landing page).
**Next action:** Andrew reviews Phase 3.0 diff, confirms hydration errors gone in webview console. CC then starts Phase 3.1 (`hooks/useShellNav.ts` + `hooks/useTabSearchParams.ts` + extend `lib/tab-store.ts` with `pendingParams` slice — pure API addition, no rendered changes).
...
**Phase 3.1-3.7 — see Key Changes below (per-phase "go" still required for each).**
