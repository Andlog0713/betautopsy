...
**Phase:** Phase 3.4a verified on iPhone (Andrew confirmed). Phase 3.4b shipped pending push: useSearchParams → useTabSearchParams refactor with per-param pattern map. 3 useRef one-shots (run, upload, unlock) + 1 early-return (sportsbook); 5 focused consume effects replaced original 2; runAutopsy extended with scopeOverride to fix closure-staleness from synchronous setAnalyzeScope+runAutopsy in the upload effect. typecheck + build:mobile clean.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's diff spot-check + iPhone test (per Andrew: most blast radius of any tab body migration so far — verify run=true now triggers autopsy on native, upload_id auto-runs with correct scope, post-checkout unlock fires pixel + runs full report, sportsbook filter applies).
