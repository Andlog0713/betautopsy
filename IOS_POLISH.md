...
**Phase:** Phase 1 (Foundation) shipped — zustand installed, `lib/tab-store.ts` + `hooks/useScrollMemory.ts` + `lib/screen-prefetch.ts` written + typecheck-clean. No rendered output yet. Awaiting Andrew's spot-check + "go" for Phase 2.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's spot-check of Phase 1 diff before Phase 2 starts (per "between phases, push and pause for review" rule from session start)
**Next action:** Andrew reviews Phase 1 diff. CC then starts Phase 2 (AppShell skeleton + TabBar component, 5 placeholder sibling divs, no real tab body migration yet).
...
**Phase 2-6 — see Key Changes below (still pending Andrew's per-phase "go").**

**Key changes (file paths use repo convention — no `src/` dir):**
- [x] Install `framer-motion@^12.38.0` (already in deps; import from `motion/react` per Motion v12) and `zustand@^4.5` *(Phase 1)*
