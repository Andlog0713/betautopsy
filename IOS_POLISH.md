...
**Phase:** Phase 3.0 verified clean on physical iPhone (React #418/#423 gone, no visible flicker). Phase 3.1 shipped pending push: tab-store extended with pendingParams slice + `useShellNav` + `useTabSearchParams` hooks + `ShellTabContext`. Pure API addition — no rendered behavior change. typecheck + build:mobile clean. No iPhone re-test needed for 3.1 per Andrew's call.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's spot-check of Phase 3.1 diff. Once approved, Phase 3.2 (Dashboard tab migration — the recipe template) starts. Per Andrew's commit-plan note: "3.2 is the recipe template — spend extra care. If 3.2 needs revisions after my review, do them BEFORE starting 3.3."
**Next action:** Andrew reviews Phase 3.1 diff + the hook signatures. CC then starts Phase 3.2 (Dashboard tab body migration to `components/tabs/DashboardTab.tsx`, page.tsx becomes 1-line re-export, replace 13 cross-tab Links with useShellNav.navigate, swap window.location.search reads for useTabSearchParams, mount in AppShell.tsx replacing Dashboard placeholder, wrap each tab section in ShellTabContext.Provider).
...
**Phase 3.2-3.7 — see Key Changes below (per-phase "go" still required for each).**
