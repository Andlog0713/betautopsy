...
**Phase:** Phase 2 + 2.5 + 2.6 + 2.7 + 2.8 shipped pending push. Phase 2.7's C fix (NO_SELECT_STYLE long-press menu suppression) appears verified; Phase 2.7's A fix (+10px paddingBottom) was insufficient against Pikkit reference — Phase 2.8 bumps to +20px. typecheck + build:mobile clean. Awaiting Andrew's re-test of A only.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's iPhone re-test of A (safe-area breathing room) against Pikkit reference at +20px. B, C, D already verified.
**Next action:** Andrew rebuilds + reruns on iPhone, eyeballs the bottom padding against Pikkit. If A passes, Phase 2 verification gate closes and CC starts Phase 3 (Phase 3 recon prerequisite: debug + fix React #418/#423 landing-page hydration errors filed under iOS-PR-2 Risks). If still cramped, easy iteration — one number to bump.
