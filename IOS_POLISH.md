...
**Phase:** Phase 2 + Phase 2.5 (touch-down haptic fix-forward) shipped pending push. AppShell + TabBar at `/shell-preview`; haptic now fires on `onTouchStart`. typecheck + build:mobile both clean. Awaiting Andrew's iPhone test before Phase 3.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's iPhone-test of `/shell-preview` (TabBar safe-area, layoutId hairline animation, touch-down haptic, display:none scroll preservation as a structural sanity check). Navigation path documented below.
**Next action:** Andrew runs `npm run ios:build && npm run ios:open`, builds + runs from Xcode, navigates the WKWebView to `/shell-preview` (via Safari Web Inspector → JS console: `location.href = '/shell-preview'`), confirms the four checks. Once confirmed, CC starts Phase 3 (migrate 5 tab page bodies into `components/tabs/<Tab>Tab.tsx` + add `useShellNav()` hook that branches on `isMobileApp()` for `<Link>` vs `useTabStore`).
