...
**Phase:** Phase 2 + 2.5 + 2.6 shipped pending push. AppShell + TabBar at `/shell-preview`; haptic on touch-down; visible top-right "Shell Preview" pill on the landing page (native-only) provides the iPhone navigation that 4 console-based strategies couldn't. typecheck + build:mobile clean. Awaiting Andrew's iPhone test (planned tomorrow morning, fresh).
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's iPhone-test of `/shell-preview` via the new top-right pill (TabBar safe-area, layoutId hairline animation, touch-down haptic, display:none scroll preservation as a structural sanity check).
**Next action:** Andrew runs `npm run ios:build && npm run ios:open`, builds + runs from Xcode, sees the "Shell Preview" pill on the landing page, taps it, confirms the four checks. Once confirmed, CC starts Phase 3 (migrate 5 tab page bodies into `components/tabs/<Tab>Tab.tsx` + add `useShellNav()` hook that branches on `isMobileApp()`). **Phase 3 has a recon prerequisite** — debug + fix the React #418/#423 landing-page hydration errors flagged during the Phase 2 iPhone test (filed under iOS-PR-2 Risks).
