...
**Phase:** Phase 3.3 verified on iPhone (Andrew confirmed). Phase 3.4a shipped pending push: ReportsTab body migration only (no useSearchParams refactor — that lands in 3.4b). Component renamed; 2 cross-tab navs migrated to TabLink; 3 /pricing `<a>` audited; useScrollMemory wired. AppShell mount swaps placeholder for ReportsTab. typecheck + build:mobile clean.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's spot-check of Phase 3.4a diff before iPhone test. Per his split plan: 3.4a proves body-only migration in isolation; 3.4b refactors the 6 useSearchParams reads with the per-param pattern map (4 useRef one-shots + 1 early-return-from-no-params for sportsbook).
...
**Phase 3.4b-3.7 — see Key Changes below (per-phase "go" still required for each).**
