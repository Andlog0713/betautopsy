...
Latest commit on branch: 215ff06 fix(ios): port ErrorBoundary wrap to ReportsTab.tsx (Phase 3.4c shipping next commit)
Latest commit on main: acb914f fix(reports): ErrorBoundary around AutopsyReport + re-apply auth cache-miss fix (#19)
Active CC session: iOS-PR-2 Phase 3.4c — UX corrections from 3.4b iPhone test (no run auto-fire, always-visible run form)
Last verified on iPhone: 2026-05-09 (Phase 3.4b — autopsy fires on cross-tab nav, re-tap doesn't re-fire, console clean)
...
**Phase:** Phase 3.4b verified on iPhone. Phase 3.4c shipped pending push: UX corrections from 3.4b iPhone test. (1) `?run=true` no longer auto-fires runAutopsy — user lands on Reports, picks scope deliberately, taps Run manually. ?upload_id and ?unlocked=true&id auto-fire preserved (specific scope intent). (2) Run form always visible at top — `&& !freeExhausted` dropped; freeExhausted upsell card + free-tier note moved below past reports list. Plus parallel ErrorBoundary commit (215ff06) ported from main's PR #19 — wraps AutopsyReport mount + dependent UI in components/tabs/ReportsTab.tsx so the Sentry-surfaced [6] crash is contained on native too. typecheck + build:mobile clean.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's iPhone test of 3.4c. Verify: (a) cross-tab nudge → reports tab does NOT auto-fire autopsy (user must tap Run manually); (b) /reports?upload_id=X cold link still auto-fires; (c) post-checkout `?unlocked=true&id=X` still auto-fires + fires conversion pixel; (d) free-tier user with 1+ reports sees form at top, can run another snapshot, sees upsell card below the list (not blocking).
**Next action:** Andrew runs iPhone test on 3.4c. If verified, Phase 3.5 (Upload tab body migration — small, mechanical) is queued.
...
**Phase 3.3 — Bets tab migration** — *shipped pending push*
- [x] `components/tabs/BetsTab.tsx`: full body migration of `app/(dashboard)/bets/page.tsx` (832 lines including BetEntryForm, ClearAllBets, SortTh helpers — kept inline, only consumed by BetsTab). 2 cross-tab TabLinks: `/reports` back-link (line ~243, conditional on `from=report`) + `/upload` empty-state CTA (line ~363). 1 out-of-shell `<a href="/pricing">` Pro upsell with PR-6 audit comment (line ~277). useSearchParams reads (`sport`, `bet_type`, `from`) migrated to a single useTabSearchParams consume effect with the verbatim Phase 3.2 recipe-rule comment + clearParams + early-return-from-no-params guard. **Variant from Phase 3.2's one-shot pattern:** Bets uses early-return-from-no-params instead of a useRef one-shot because the consume can fire multiple times legitimately (user navigates with `?sport=NFL`, later with `?sport=NBA` etc.). The `?from=report` flag is captured into local state (`showBackLink`) so the back-link survives clearParams. `useScrollMemory<HTMLDivElement>('bets')` ref attached to outer scroll container.
- [x] `app/(dashboard)/bets/page.tsx`: 1-line re-export.
- [x] `components/shell/AppShell.tsx`: bets panel mounts `<BetsTab />`. PLACEHOLDER record now `Exclude<TabId, 'dashboard' | 'bets'>` — 3 placeholders remaining (reports, upload, settings).
- [x] Typecheck clean. Build:mobile verification before commit.

**Phase 3.4a — Reports body-only migration (split per Andrew's pre-emptive 3.4a/3.4b plan)** — *shipped pending push*
- [x] `components/tabs/ReportsTab.tsx`: full body migration of `app/(dashboard)/reports/page.tsx` (1025 lines including the 2 inline helpers — AnalyzingState, AnalyzingProgress — kept inline). Component renamed `ReportsPage` → `ReportsTab`.
- [x] **2 cross-tab nav migrations only:** Line 523 `<Link href="/dashboard">` (post-first-report prompt) → `<TabLink to="dashboard">`; Line 773 `<a href="/upload">` (empty-state CTA) → `<TabLink to="upload">` (Andrew's recon correction — original recon flagged it as raw `<a>` but it's a tab destination, same pattern as DashboardTab's empty-state method cards).
- [x] **3 raw `<a href="/pricing">` retained** with PR-6 audit comments at the unlock-full-report upsell sites (~ lines 492, 593, 624 of original → similar lines in migrated file). Same pattern as DashboardTab line 723 + BetsTab line 277.
- [x] `useScrollMemory<HTMLDivElement>('reports')` ref attached to all 3 outer scroll containers (loading return, activeReport return, default return — each now wraps with `<div ref={scrollRef} className="flex-1 overflow-y-auto">`).
- [x] `app/(dashboard)/reports/page.tsx`: 1-line re-export.
- [x] `components/shell/AppShell.tsx`: reports panel mounts `<ReportsTab />`. PLACEHOLDER record narrowed to `Exclude<TabId, 'dashboard' | 'bets' | 'reports'>` — 2 placeholders remaining (upload, settings).
- [x] **NOT in 3.4a (deferred to 3.4b):** all 6 useSearchParams reads (`run`, `upload_id`, `upload_ids`, `sportsbook`, `unlocked`, `id`) stay using `next/navigation`'s useSearchParams. No consume effect refactors. No recipe-rule comments. Per-param pattern map for 3.4b: `run` / `upload_id` / `upload_ids` / `unlocked` / `id` use useRef one-shot (fire-once intents); `sportsbook` uses early-return-from-no-params (re-applicable filter). On native today, cross-tab `navigate('reports', {run:'true'})` activates the tab but does NOT trigger autopsy — that wires up in 3.4b.
- [x] Typecheck clean. Build:mobile verification before commit.

**Phase 3.4b — Reports URL param refactor (per-param pattern map applied)** — *shipped pending push*
- [x] `components/tabs/ReportsTab.tsx`: replaced `useSearchParams` with `useTabSearchParams` + added `useShellNav` (for `clearParams`) + `isMobileApp`. Single `searchParams` declaration replaced with `tabParams` + `clearParams` destructure.
- [x] **3 useRef one-shots** + 1 early-return-from-no-params, per Andrew's spec: `runConsumedRef` (run=true), `uploadConsumedRef` (upload_id OR upload_ids — alternative encodings of same intent), `unlockConsumedRef` (unlocked=true paired with id). Sportsbook stays re-applicable via early-return.
- [x] **Replaced 2 original effects (mount + auto-run) with 5 focused effects:**
  1. `loadReports` mount-only (unchanged, no params consumed)
  2. **Sportsbook scope-set** (early-return guard, `setAnalyzeScope('book:X')`, no replaceState)
  3. **Upload scope-set + autopsy** (one-shot via `uploadConsumedRef`, sets scope, fires runAutopsy with `scopeOverride` for upload_id only — original asymmetry preserved: upload_ids sets scope but doesn't auto-run)
  4. **Run autopsy** (one-shot via `runConsumedRef`, fires runAutopsy on current scope)
  5. **Post-checkout unlock** (one-shot via `unlockConsumedRef`, captures paidId from URL, fires conversion pixel, fires runAutopsy with `paidIdOverride`)
- [x] Each consume effect carries the verbatim Phase 3.2 recipe-rule comment block + `clearParams('reports')` + branched `if (!isMobileApp()) replaceState` (where applicable — sportsbook doesn't replaceState, matching original mount-effect behavior).
- [x] **`runAutopsy` extended with `scopeOverride` parameter.** Required because the upload effect calls `setAnalyzeScope('upload:X')` and then `runAutopsy(undefined, 'upload:X')` in the same effect body — without the override, the closure would capture the OLD `analyzeScope` (React state hasn't applied yet) and the API call would go out with no upload_id. The override path is taken only on upload-effect-driven autopsy fires; manual Run button + run/unlock effects pass `undefined` and fall back to closure's `analyzeScope` (which is correct because they don't mutate scope synchronously). Body construction now uses `effectiveScope = scopeOverride ?? analyzeScope`.
- [x] **Edge case accepted (Andrew 2026-05-09):** if multiple fire-once params arrive on the same navigate (e.g. `?unlocked=true&run=true`), each effect fires runAutopsy independently → multiple API calls. Realistic-likelihood is ~0 — the URL flows that produce these params produce one at a time (post-checkout = unlocked+id alone; CTA = run alone or upload_id alone). Documented in the consume-effect refs comment block.
- [x] Typecheck clean. Build:mobile verification before commit. iPhone test required after push (most blast radius of any tab body so far per Andrew).

**Phase 3.4c — UX corrections from 3.4b iPhone test** — *shipped pending push*
- [x] **`?run=true` no longer auto-fires `runAutopsy`.** Per Andrew's product call: BetAutopsy is a behavioral analysis tool, not an instant-results gambler app. User should land on Reports, pick scope (date range, sport filter, upload selection), then tap Run manually. The deliberate moment IS the value. The run consume effect still consumes the param (sets `runConsumedRef.current = true`, calls `clearParams('reports')`, replaceState on web) — only the `runAutopsy()` call is removed.
- [x] **`?upload_id` and `?unlocked=true&id` auto-fire preserved.** Those carry specific scope intent (chosen upload, paid snapshot) and the user is already past the deliberation step when they arrive at the URL. Only the standalone-trigger `?run=true` path changes.
- [x] **Run form ALWAYS visible at top.** `&& !freeExhausted` dropped from the form's render conditional. Form (Analyze controls card) renders for any `totalBetCount > 0 && !running` user. Free-exhausted upsell card + free-tier note moved BELOW the past reports list — still rendered for free users with 1+ reports (preserving conversion pressure), but no longer block access to running another snapshot.
- [x] **New layout order**: Header → Form → Running state / Empty state / Past Reports → Free-tier note → Free-exhausted upsell. `PRICING_ENABLED=false` in prod today means user-visible diff is zero; forward-compatible work for when pricing flips back on.
- [x] **Dropdown styling deferred to PR-9** (visual identity work).
- [x] Typecheck clean. Build:mobile verification before commit. iPhone test required after push.

**Phase 3.5-3.7 — see Key Changes below (per-phase "go" still required for each).**

**Cleanup item for Phase 3.7 (filed 2026-05-09):** Each migrated tab body wraps its content in `<div ref={scrollRef} className="flex-1 overflow-y-auto">`, AND PerTabContainer in AppShell ALSO wraps with the same pattern. Two `useScrollMemory(tabId)` calls per tab → one in PerTabContainer (active scroll container, listeners fire), one in tab body (inert: parent is a regular block, flex-1 doesn't apply, no scroll triggered). Redundant but functionally correct — outer container is the actual scroll surface, inner is dead weight. Phase 3.7 should remove the inner scrollRef/wrap from each tab body so PerTabContainer is the only scroll container per tab. Behavioral change: zero (already only outer scrolls). LOC reduction: ~3 lines per tab × 5 tabs = 15 lines. Not worth a fix-forward in 3.3-3.6 — defer to integration phase to avoid disrupting per-tab bisect points.
...
2026-05-09 · iOS-PR-2 Phase 3.4b · Parallel-session collision. Two CC sessions worked the same prompt independently on `claude/ios-pr2-app-shell`. Session A (committed `01bfe9f`) had context on the `upload_ids` asymmetry — original code does NOT auto-fire runAutopsy on multi-upload selection; user picks scope then taps Run manually. Session B (this session, local-only `8c9308a`, discarded) treated `upload_id` and `upload_ids` symmetrically and would have auto-fired multi-upload analysis on cold link — a real product regression. Other notable design splits: Session A extended `runAutopsy` signature with `scopeOverride` (smaller surface change); Session B introduced `pendingAutoRun` local state + 6-effect split (more mechanism, fewer regression vectors but also misaligned with original behavior). Session B did Phase 0 recon + planning + coding for work that was already done because the user's prompt asserted "previous CC session FAILED on the 3.4b prompt (never executed)" without first verifying origin's HEAD. `git fetch && git log --oneline origin/<branch>` BEFORE the recon would have caught it in 2 seconds. Resolution: kept `01bfe9f`; Session B's local commit reset to origin. Lessons: (a) WORKFLOW PROTOCOL "One PR at a time, no parallel CC sessions" was violated — two sessions on the same PR. Origin had no protection against this, and neither session detected the other until push collision. (b) Every CC session must `git fetch origin <branch>` and verify HEAD against the user's claim of state BEFORE Phase 0 recon, not after. "Previous session FAILED" is a hypothesis; origin's HEAD is the truth. (c) When two designs implement the same spec, the one with deeper context on existing product behavior wins. `upload_ids` asymmetry was a behavior preserved by reading the original code carefully — Session A did, Session B's prompt didn't include the constraint and the design diverged.
```
...
## PRE-EXISTING NATIVE ROUTING BUGS (deferred to PR-6)

Capacitor WKWebView routing quirks that surface during iOS PR work but aren't introduced by it. Distinct from web bugs because they're native-only. PR-6 handles these as part of the cross-shell-nav UX regression cleanup (sheet/PageStack conversion).

```
2026-05-09 (surfaced during iOS-PR-2 Phase 3.2 iPhone test) · Capacitor `<a href="/route">` quirk · Raw <a> elements with same-origin href (e.g. `<a href="/pricing">` from DashboardTab's Start Tracking CTA) don't always navigate properly inside the Capacitor WKWebView. Same quirk Andrew hit in Phase 2.6 attempting `location.href = '/shell-preview'` from the Web Inspector console — required a TabLink-rendered `<Link>` to bypass. The Capacitor `iosScheme: https` + `hostname: localhost` setup apparently has same-origin routing semantics that raw <a> doesn't satisfy reliably.
Status: not blocking — /pricing CTAs are out-of-shell and will become PageStack pushes / sheets in PR-6 anyway, at which point the raw <a> goes away. PR-6 audit comments at every <a href="/pricing"> site (DashboardTab line ~723, BetsTab line ~277, future tabs as they migrate) flag these for conversion.
```
...
2026-05-09 · iOS-PR-2 Phase 3.2 verification · claude/ios-pr2-app-shell · Andrew confirmed Phase 3.2 verified on physical iPhone. Dashboard tab renders under the shell with real data; tab switching preserves scroll position; cross-tab CTAs from Dashboard correctly fire navigate (don't unmount AppShell); no React #418/#423 regressions in webview console. /pricing CTA tap surfaced a Capacitor `<a href="/route">` routing quirk — same family as the Phase 2.6 console-nav blocker — logged under new "PRE-EXISTING NATIVE ROUTING BUGS" section, deferred to PR-6 (raw <a> goes away when /pricing converts to a sheet/PageStack push). Trackpad two-finger scroll bug on /dashboard surfaced during web spot-check; confirmed pre-existing on production live betautopsy.com, NOT caused by Phase 3.2 — logged under "PRE-EXISTING WEB BUGS" for PR-3 recon. · (no commit — verification report only)
2026-05-09 · iOS-PR-2 Phase 3.3 verification · claude/ios-pr2-app-shell · Andrew confirmed Phase 3.3 verified on physical iPhone. Tests 1-3 + 5 pass (Bets list renders under shell, filter chips work, cross-tab "Upload CSV" empty-state nav fires upload tab activate, console clean). Test 4 (`useTabStore.setState` console manipulation for `from=report` simulation) was a bad test — useTabStore isn't accessible from Web Inspector console (correct module isolation). Indirect verification via Tests 1-3 covers the same plumbing. · (no commit — verification report only)
2026-05-09 · iOS-PR-2 Phase 3.4a verification · claude/ios-pr2-app-shell · Andrew confirmed Phase 3.4a verified on physical iPhone. Real reports render under shell; cross-tab navs stay in shell; ?run=true correctly does NOT auto-fire on native (3.4b deferral working as designed); in-tab interactions work; console clean. · (no commit — verification report only)
2026-05-09 · iOS-PR-2 Phase 3.4b · claude/ios-pr2-app-shell · Reports URL param refactor with per-param pattern map. components/tabs/ReportsTab.tsx: useSearchParams → useTabSearchParams + useShellNav (clearParams) + isMobileApp imports. 3 useRef one-shots (runConsumedRef for run=true, uploadConsumedRef shared by upload_id/upload_ids, unlockConsumedRef for unlocked=true paired with id) + 1 early-return-from-no-params for sportsbook (re-applicable filter). Original 2 effects (mount + auto-run) replaced with 5 focused consume effects: (1) loadReports mount-only, (2) sportsbook scope-set early-return, (3) upload scope-set + autopsy one-shot — fires runAutopsy with scopeOverride for upload_id (preserves original asymmetry: upload_ids sets scope but doesn't auto-run), (4) run autopsy one-shot, (5) post-checkout unlock paired one-shot — captures paidId from URL synchronously, fires conversion pixel, runAutopsy with paidIdOverride. Each consume effect carries verbatim Phase 3.2 recipe-rule comment + clearParams('reports') + branched `if (!isMobileApp()) replaceState` (sportsbook skips replaceState matching original mount-effect behavior). runAutopsy extended with optional `scopeOverride: string` parameter — fixes closure-staleness in upload effect where synchronous `setAnalyzeScope + runAutopsy` would otherwise read OLD analyzeScope from closure; body construction uses `effectiveScope = scopeOverride ?? analyzeScope`. Edge case (accepted): simultaneous arrival of multiple fire-once params triggers multiple runAutopsy calls; realistic-likelihood ~0 because URL flows produce one fire-once param at a time. Typecheck clean; build:mobile clean. · (this commit)
2026-05-09 · iOS-PR-2 Phase 3.4a · claude/ios-pr2-app-shell · Reports body-only migration per Andrew's pre-emptive 3.4a/3.4b split plan. New file: components/tabs/ReportsTab.tsx (1025-line page.tsx body + 2 inline helpers AnalyzingState/AnalyzingProgress kept inline). Function renamed ReportsPage → ReportsTab. 2 cross-tab nav migrations: line ~523 `<Link href="/dashboard">` → TabLink (post-first-report prompt), line ~773 `<a href="/upload">` → TabLink (empty-state CTA, per Andrew's recon correction — tab destination, not raw <a>). 3 `<a href="/pricing">` upsell sites retained with PR-6 audit comments matching DashboardTab + BetsTab pattern. useScrollMemory<HTMLDivElement>('reports') ref attached to all 3 outer scroll containers (loading return, activeReport return, default return). app/(dashboard)/reports/page.tsx becomes 1-line re-export. AppShell.tsx mounts <ReportsTab /> in reports panel; PLACEHOLDER record narrowed to 2 remaining (upload, settings). NOT migrated in 3.4a (deferred to 3.4b): all 6 useSearchParams reads (run, upload_id, upload_ids, sportsbook, unlocked, id), no consume effect refactors, no recipe-rule comments. On native today, cross-tab navigate('reports', {run:'true'}) activates the tab but does NOT trigger autopsy — that wires up in 3.4b with per-param pattern map (4 useRef one-shots for fire-once flags, 1 early-return-from-no-params for sportsbook filter). Typecheck clean; build:mobile clean. · (this commit)
2026-05-09 · iOS-PR-2 Phase 3.3 · claude/ios-pr2-app-shell · Bets tab migration following the Phase 3.2 recipe template. New file: components/tabs/BetsTab.tsx (full body migration of 832-line page.tsx including the 3 inline helpers — BetEntryForm, ClearAllBets, SortTh). 2 cross-tab TabLinks (`/reports` back-link, `/upload` empty-state CTA), 1 out-of-shell `<a href="/pricing">` Pro upsell with PR-6 audit comment (matches DashboardTab's pattern). useSearchParams reads (`sport`, `bet_type`, `from`) migrated to a single useTabSearchParams consume effect with the verbatim Phase 3.2 recipe-rule comment + clearParams + early-return-from-no-params guard. Variant from Phase 3.2's pattern: Bets uses early-return guard instead of useRef one-shot because filter consume can fire multiple times legitimately (user navigates with `?sport=NFL` then later `?sport=NBA`). The `?from=report` flag captured into local state (`showBackLink`) so the back-link survives clearParams. useScrollMemory<HTMLDivElement>('bets') ref attached to outer scroll container. app/(dashboard)/bets/page.tsx becomes 1-line re-export. AppShell.tsx mounts <BetsTab /> in bets panel; PLACEHOLDER record narrowed to 3 remaining placeholders. Cleanup item filed for Phase 3.7: each migrated tab body wraps with scrollRef + overflow-y-auto, AND PerTabContainer also wraps with the same — redundant useScrollMemory call per tab (inner is inert because parent block isn't flex), defer cleanup to integration phase to avoid disrupting per-tab bisect points. Capacitor <a href="/route"> quirk logged under new top-level "PRE-EXISTING NATIVE ROUTING BUGS" section per Andrew's instruction (separate from PRE-EXISTING WEB BUGS — different fix track, queued for PR-6). Typecheck clean; build:mobile clean. · (this commit)
...
2026-05-09 · iOS-PR-2 Phase 3.4b verification + ErrorBoundary port · claude/ios-pr2-app-shell · Andrew confirmed 3.4b on iPhone (cross-tab nav fires autopsy on Reports, re-tap doesn't re-fire, console clean). Separately, main shipped a class-based ErrorBoundary in PR #19 (acb914f) wrapping AutopsyReport in app/(dashboard)/reports/page.tsx — contains a Sentry-surfaced runtime crash on /reports for post-checkout users (`?unlocked=true&id=X`): "Cannot read properties of undefined (reading '6')" inside the AutopsyReport render tree, bleeding into React #310 ("rendered more hooks") as the page unwound. Ported same change here on PR-2 branch: components/ErrorBoundary.tsx (identical class component) + ErrorBoundary wrap in components/tabs/ReportsTab.tsx (same scope: AutopsyReport + post-first-report prompt + ProUpsellModal inside; Back button + tierLimited card outside so user can always escape). Underlying [6] crash root cause not fixed in this commit — likely recharts / data-shape divergence in post-checkout flow where reports[1] is the user's old snapshot with trimmed report_json shape; deferred to a separate diagnosis pass. Sentry release tag 27a2d3d1373f confirmed pre-auth-fix, so the crash is pre-existing and not caused by recent auth work. Typecheck clean; build:mobile clean. Not merging this branch — queued for iOS-PR-2 ship. · 215ff06
2026-05-09 · iOS-PR-2 Phase 3.4c · claude/ios-pr2-app-shell · UX corrections from 3.4b iPhone test. Two changes in components/tabs/ReportsTab.tsx. (1) ?run=true no longer auto-fires runAutopsy. Per Andrew's product call: BetAutopsy is a behavioral analysis tool, not an instant-results gambler app — user should land on Reports, pick scope (date range, sport filter, upload selection), then tap Run manually. The deliberate moment IS the value. The run consume effect still consumes the param (sets runConsumedRef=true, calls clearParams + replaceState) so a stale ?run=true doesn't re-arm; only the runAutopsy() call is removed. ?upload_id and ?unlocked=true&id auto-fire stays in place — those carry specific scope intent (chosen upload, paid snapshot) and the user is past deliberation. (2) Run form always visible at top. `&& !freeExhausted` dropped from the form's render conditional; the freeExhausted upsell card + free-tier note moved BELOW the past reports list. Layout order: Header → Form → Running/Empty/PastReports → free-tier note → freeExhausted upsell. Free user with 1+ reports can now run another snapshot without deleting old ones; upsell still renders for conversion pressure but no longer blocks work. PRICING_ENABLED=false in prod today → user-visible diff is zero; forward-compatible work for when pricing flips back on. Dropdown styling deferred to PR-9 (visual identity). Typecheck clean; build:mobile clean. iPhone test required after push. · (this commit)
```
