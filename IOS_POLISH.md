...
**Phase:** Phase 3.1.5 spot-check returned three checks; 3.1.6 ships the fix-forward. setActive refactored to true early-return via `get()` (removes implicit dependency on zustand v4's identity bailout, survives library version bumps) + edge-case comment near dedup. typecheck + build:mobile clean. No iPhone re-test needed.
**Branch:** `claude/ios-pr2-app-shell`
**Blocking:** Andrew's go on Phase 3.2 (Dashboard tab migration — the recipe template).
