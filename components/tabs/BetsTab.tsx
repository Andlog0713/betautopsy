...
export default function BetsTab() {
  ...
  // Scroll memory ref. Inert on web (no constrained parent → no inner
  // scroll triggered); active on native (AppShell's section bounds
  // height). Same pattern as DashboardTab from Phase 3.2.
  const scrollRef = useScrollMemory<HTMLDivElement>('bets');
  ...
  // `from=report` was previously read directly from useSearchParams in
  // the JSX. Phase 3.3 captures it into local state at consume time so
  // the back-link survives clearParams (which we MUST call per the
  // Phase 3.2 recipe rule). On web /bets, the URL retains ?from=report
  // until navigation; on native, pendingParams clears after consume so
  // local state is the only persistent source.
  const [showBackLink, setShowBackLink] = useState(false);

  // ── Consume effect for ?sport / ?bet_type / ?from params ──
  ...
  //
  // Bets uses the no-one-shot variant (vs DashboardTab's welcomeConsumedRef
  // pattern) because the consume can fire multiple times legitimately:
  // user navigates from /reports with ?from=report, later from another
  // tab with ?sport=NBA, etc. The early-return from "nothing to consume"
  // prevents the re-render loop after clearParams.
  useEffect(() => {
    const sp = tabParams.get('sport');
    const bt = tabParams.get('bet_type');
    const from = tabParams.get('from');
    if (!sp && !bt && !from) return;

    if (sp) setSportFilter(sp);
    if (bt) setTypeFilter(bt);
    if (from === 'report') setShowBackLink(true);

    clearParams('bets');
    // Note: original code did NOT replaceState the URL on web. Leaving
    // that behavior unchanged — the URL retains ?sport=...&from=...
    // until the user navigates elsewhere. Browser back/forward replay
    // works as expected.
  ...
  return (
    <div ref={scrollRef} className="flex-1 overflow-y-auto">
      ...
      {showBackLink && (
        <TabLink to="reports" className="text-sm text-fg-muted hover:text-fg transition-colors">
          ← Back to Report
        </TabLink>
      )}
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <TabLink to="upload" className="btn-primary text-sm">Upload CSV</TabLink>
          </div>
        </div>
      ) : (
