...
import { FlaskConical, Brain, Flame, Dice5, DollarSign, Eye, Upload, PenLine, Lock, Snowflake, AlertTriangle } from 'lucide-react';
...
// Cross-tab nudge target. iOS-PR-2 Phase 3.2 migration: replaced
// the original `{ href: string }` shape with `{ to, params? }` so
// the nudge consumer can route through TabLink (in-shell on native)
// instead of <Link> (which on native would unmount AppShell).
interface Nudge {
  icon: ReactNode;
  message: string;
  action: string;
...
export default function DashboardTab() {
  ...
  // Scroll memory for the dashboard tab. The ref attaches to the
  // outer scroll container below; on native, AppShell's section
  // bounds the height so this div is the actual scroll surface,
  // and useScrollMemory rAF-restores scrollTop on re-activation.
  // On web /dashboard, document body handles scrolling and
  // overflow-y-auto is inert (the div has no fixed height), so
  // useScrollMemory is harmless — it just attaches scroll/restore
  // handlers that never fire.
  const scrollRef = useScrollMemory<HTMLDivElement>('dashboard');
  ...
  // One-shot guards for the consume effects. Without these, a
  // re-render after clearParams would re-fire the analytics pixel
  // because the consume effect's deps changed (tabParams went from
  // {welcome:'true'} to {}).
  const welcomeConsumedRef = useRef(false);
  const upgradedConsumedRef = useRef(false);
  ...
  useEffect(() => {
    if (welcomeConsumedRef.current) return;
    if (tabParams.get('welcome') !== 'true') return;
    welcomeConsumedRef.current = true;
    ...
    // Recipe-rule cleanup: clearParams clears native pendingParams;
    // replaceState scrubs the URL on web. Both run unconditionally;
    // each is moot on the other platform (clearParams is no-op on
    // web per useShellNav design, replaceState rewrites a URL
    // nobody reads on native).
    clearParams('dashboard');
    if (!isMobileApp()) {
      window.history.replaceState({}, '', '/dashboard');
    }
  }, [tabParams, clearParams]);

  // Separate effect for the welcome-pulse timeout — split from the
  // consume effect so the cleanup (clearTimeout) doesn't fire on
  // tabParams changes that happen DURING the pulse. Deps depend
  // only on welcomePulse so the timeout lives until the pulse ends.
  useEffect(() => {
  ...
  // ── Upgraded consume effect ──
  // Phase 3.2 recipe rule (same as welcome above) — pulled out of
  // the stats-loading effect where it used to live inline (line 156
  // in the pre-migration page.tsx). Fires once when ?upgraded=true
  // arrives in tabParams AND profile is loaded so `tier` is current.
  useEffect(() => {
    if (upgradedConsumedRef.current) return;
    if (tabParams.get('upgraded') !== 'true') return;
    if (!user || !profile) return;
    upgradedConsumedRef.current = true;
  ...
  }, [tabParams, user, profile, tier, clearParams]);
  ...
  }, [user, userLoading, reportsLoading, reports, reportCount, reloadToken]);
  ...
  // Phase 3.2 migration: nudge.href → nudge.to + nudge.params so the
  // consumer at line ~ uses TabLink instead of <Link>.
    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto">
      ...
      {stats && stats.reportCount === 0 && (
        <div className="case-card p-6 mb-8">
          ...
          <div className="space-y-0">
            ...
            <div className="flex items-center gap-3">
              ...
              {hasBets ? (
                ...
                <TabLink to="upload" className="font-mono text-sm text-scalpel hover:underline">Upload your betting history</TabLink>
              )}
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-5 h-5 rounded-sm border-2 border-border-subtle bg-transparent shrink-0" />
              {hasBets ? (
                <TabLink to="reports" className="font-mono text-sm text-scalpel hover:underline">Run your first autopsy</TabLink>
              ) : (
          ...
          <div className="mt-6">
            {hasBets ? (
              <TabLink to="reports" className="btn-primary">Run Your First Autopsy</TabLink>
            ) : (
              <TabLink to="upload" className="btn-primary">Upload Your Bets</TabLink>
            )}
          </div>
        </div>
      )}
        ...
        <>
          ...
          {nudge && (
            <div className="flex items-center gap-3 border-l-2 border-l-scalpel pl-4 py-2 mb-10">
              ...
              <TabLink
                to={nudge.to}
                params={nudge.params}
                className="ml-auto text-sm text-scalpel link-underline whitespace-nowrap font-mono"
              >
                {nudge.action} →
              </TabLink>
            </div>
            ...
            <section className="border-t border-white/[0.04] pt-10 mb-12">
                    ...
                    <div className="lg:col-span-5 lg:border-l lg:border-white/[0.04] lg:pl-12 space-y-6">
                      ...
                      {bankroll && (
                        <div>
                          ...
                          <TabLink to="settings" className="case-header link-underline mt-2 inline-block">EDIT →</TabLink>
                        </div>
                      )}
              ...
              {stats.reportCount === 0 && (
                <div className="card-hero py-12 px-8">
                  ...
                  <TabLink to="reports" params={{ run: 'true' }} className="btn-primary inline-block text-base !px-6 !py-3 font-mono">
                    Run Your Autopsy →
                  </TabLink>
                </div>
              )}
              ...
              {isPaid && !latest && stats.reportCount > 0 && (
                <div className="border-l-2 border-l-scalpel pl-5 py-2">
                  ...
                  <TabLink to="reports" params={{ run: 'true' }} className="btn-primary inline-block text-sm font-mono">Run Autopsy</TabLink>
                </div>
              )}
            </section>
          ) : null}
          ...
          {!isPaid && (
            <section className="border-t border-white/[0.04] pt-10 mb-12">
              ...
              <a href="/pricing" className="btn-primary inline-block text-sm font-mono">Start Tracking</a>
            </section>
          )}
          ...
          <section className="border-t border-white/[0.04] pt-10">
            ...
            <div className="max-w-md">
              <TabLink to="upload" className="interactive-row flex items-center gap-3 text-sm text-fg-muted hover:text-fg-bright py-3">
                <Upload size={14} /> <span>Upload new bets</span>
              </TabLink>
              <TabLink to="reports" className="interactive-row flex items-center gap-3 text-sm text-fg-muted hover:text-fg-bright py-3">
