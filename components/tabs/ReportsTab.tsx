...
export default function ReportsTab() {
  ...
  // Scroll memory for the reports tab. Same pattern as DashboardTab
  // and BetsTab — inert on web (parent block isn't flex-constrained,
  // overflow-y-auto doesn't trigger), active on native (PerTabContainer
  // is the actual scroll surface; this wrap is currently redundant per
  // the Phase 3.7 cleanup item filed in IOS_POLISH.md).
  const scrollRef = useScrollMemory<HTMLDivElement>('reports');
    ...
    return (
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        ...
        {/* Wraps AutopsyReport + dependent UI (post-first-report prompt,
            ...
            Mirrors main's `acb914f` fix on app/(dashboard)/reports/page.tsx
            so the iOS-PR-2 tab body has the same crash containment when
            this branch ships. */}
        <ErrorBoundary>
          ...
          {isFirstReport && (
            <div className="card p-5 text-center space-y-2">
              ...
              <TabLink to="dashboard" className="text-sm text-scalpel hover:underline">
                Go to Dashboard →
              </TabLink>
            </div>
          )}
    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto">
      ...
      {totalBetCount === 0 && !running && (
        <div className="card p-12 text-center">
          ...
          <TabLink to="upload" className="btn-primary inline-block">
            Upload Bets
          </TabLink>
        </div>
      )}
