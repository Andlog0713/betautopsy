...
===`;
  ...
  const analysisBase: AutopsyAnalysis = {
    ...
    // What-If counterfactuals (lib/engine/whatIf.ts). Transforms the engine's
    // existing metrics.what_ifs into the wire array so the LLM narrative and the
    // iOS WhatIfCard share one source of truth. Full reports only — runSnapshot
