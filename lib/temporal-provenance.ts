...
const UNPROVABLE_RESULT_SEQUENCE_CLAIM = /(?:\b(?:after|follow(?:ing|ed|s)?)\s+(?:(?:a|an|the|your)\s+)?(?:loss(?:es)?|win(?:s)?|losing|winning)\b|\b(?:after|follow(?:ing|ed|s)?)\s+(?:(?:a|an|the|your)\s+)?(?:string|streak|run|series|stretch)\s+of\s+(?:loss(?:es)?|win(?:s)?)\b|\balready\s+(?:in the red|down|losing)\s+before\b|\bpost[\s-]?loss\b|\b(?:loss(?:es)?|win(?:s)?|result(?:s)?|miss(?:es)?)\s+(?:(?:appeared|seemed)\s+to\s+)?(?:caus(?:e|es|ed)|trigger(?:s|ed)?|dr(?:ive|ives|ove)|make|makes|made|push(?:es|ed)?|lead(?:s)?|led|prompt(?:s|ed)?)\b|\b(?:loss(?:es)?|win(?:s)?)\s+(?:create|creates|created|cause|causes|caused|trigger|triggers|triggered)\b|\b(?:in response to|because of)\s+(?:(?:a|an|the|your)\s+)?(?:loss(?:es)?|win(?:s)?)\b)/i;
