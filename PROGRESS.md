...
- (c) Stripe checkout opens in SafariViewController with visible URL chrome — **partially verified.** Web flow confirmed working: tester subscribed via the production website on a fresh account, got redirected to `checkout.stripe.com`. iOS SafariViewController open-with-URL-chrome still needs visual confirmation on the simulator (first iOS attempt stuck on "Redirecting..." because the test account had an existing active subscription, not because of any Capacitor/Stripe wiring issue). Same retest on a free-tier account post-rebuild expected to pass since the web path proves `/api/checkout` → Stripe URL is working end-to-end. Pricing handlers also hardened (commit `08d8ed2`): all three (`handleSubscribe`, `handleBuyReport`, `handleManage`) now surface the route's actual error message via toast on every failure path and reset `loadingAction` in `finally` — so any future "stuck Redirecting..." class of bug shows the user the real error instead of freezing silently.
