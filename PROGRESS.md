...
### Done this session
- **App Store Connect submission docs drafted** in `docs/app-store/`:
  - `PRIVACY_NUTRITION_LABELS.md` — Apple App Privacy answers, sourced from the actual codebase (Sentry config, GA4, Supabase schema, Stripe customer ID flow). Tracking = No (Meta + TikTok pixels gated by `!isMobileBuild()` at compile time, verified in `app/layout.tsx:170`). Six "Data Linked to You" categories disclosed (Email, User Content = bet history, Identifiers, Usage Data, Crash Data, Performance Data, Financial Info subscription state). Pre-submission checklist included.
  - `APP_REVIEW_NOTES.md` — copy-paste block for the App Review Information field. Pre-empts the three highest-risk questions: test credentials (two seeded review accounts), why no IAP (Guideline 3.1.3(b) Multiplatform Services — same account spans web + iOS, payments happen on web before/after install, no in-app purchase prompts), and Anthropic disclosure (Guideline 5.1.2(i) — first-launch consent modal already shipping). Also covers Guideline 5.3.4 sports-betting positioning (harm-reduction framing, not bet placement, age 17+ rating with Frequent/Intense Simulated Gambling = Yes) and 5.1.1(v) account-deletion mechanics.
  - `SCREENSHOT_SHOT_LIST.md` — six required slots with exact iPhone 6.9" sizing (1290×2796), per-slide headline + phone-screen + callout + bleed-red focus, optional 7th social-proof slide. Production pipeline uses `xcrun simctl io booted screenshot` for the actual UI capture (App Review rejects screenshots that misrepresent the app — AI image generation alone is high-risk), composited into an AI-generated frame in Figma. Style invariants enforce the BetAutopsy palette + type rules.
