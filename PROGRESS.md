...
- (c) Stripe checkout opens in SafariViewController with visible URL chrome — **passed.** Tester confirmed visually on the iOS simulator: SFSafariViewController slid up over the BetAutopsy app, `checkout.stripe.com` rendered in the URL bar at the top, native iOS browser chrome present (X close button top-left, native bottom toolbar with share/reload/compass, "BetAutopsy" merchant name inside Stripe's hosted form, payment methods rendering natively, email pre-filled). Satisfies Guidelines 3.1.1 / 3.1.3 — payment is unambiguously web-routed, not in-app. Pricing handlers hardened earlier in `08d8ed2` (toast on every failure path, `finally` for state reset) — any future error will now surface to the user instead of freezing on "Redirecting...".
